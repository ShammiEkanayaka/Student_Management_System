...
module.exports = {    //create module.
   ...
   getUsers: (req, res) => {   
    ...
    login: (req, res) => {
        var body = req.body;
        getUserByUserGrade(body.email, (err,results) => {
            if (err) {
                console.log(err);
            }
            if (!results) {
                return res.json ({
                    success: 0,
                    data: "Invalid email or pasword"
                });
            }
            var result = compareSync(body.password, results.password);
            if (result){
                results.password = undefined;
                var jasontoken = sign({ result: results}, "qwe1234", {   //??????????? get "qwe1234" value by .env file without hardcoding.  
                    expiresIn: "1h"
                });
                return res.json({
                    success:1,
